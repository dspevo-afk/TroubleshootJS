...
**Current uniqueness closure:** The later menu-wide qualification adds a meaningful postvalidation physical fingerprint, bounded successful-board history and bounded fresh-root history. All nine menu pairs pass 20 structural seeds each with distinct merged copper and placement, while 27 real successive launches and two exact replays pass on the production build. The raw candidate corpus still reports typed rejects and does not imply universal seed success. [Current acceptance packet](task-evidence/QuickPlayUnique/README.md).
