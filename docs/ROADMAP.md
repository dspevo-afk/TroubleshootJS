...
**Status:** COMPLETE — typed differential DC/AC outcomes, reference refusal, and shared accepted-step solver-time observation are qualified (2026-09-20). Every player-visible U02 DMM voltage path carries the real finite `$10 MOhm$` burden after reference preflight; there is no unloaded/live-voltage exception. Retained DC and AC probes reacquire only after bounded accepted solver time, never repaint cadence. AC RMS is bounded, AC-coupled, and separately qualifies observed content against its declared 200 Hz passband using complete observed cycles, including phase-varied near-cutoff coverage. Contracts cover high-source-impedance loading, irregular samples, DC-offset removal, clipping, reference failures before a load is installed, cleanup, and stale acquisition invalidation; built-player evidence uses visible red/black probes plus developer-only temporal CircuitJS fixtures. [Evidence](task-evidence/U02-U03/README.md).
