...
The E02 electrical interpretation and E04 physical topology changed in this
repair. The diagnostic context advances from dependency-v15/dump-model-v4 to
dependency-v16/dump-model-v5; an older value-only proof cannot compare equal to
the repaired context. Focused D01/dependency tests pin the framed epoch. Cold
serial evidence, static partitions, publication ordering and fresh warm owner/
controller receipts remain the authoritative D01 architecture.
...
The rebuilt final-source compiled A10 generation verifier ran actual cold and
warm proof/cache paths through `GenerationCoordinator`; the independent
`a10_generation_report.py`
...
bounded work and stable cold/warm values. Its [compiled browser report](a10-generation-report.json)
is **PASS**: 3,128 assertions, 24 frozen normal-corpus rows and eight D01
...
`D01_JOB_BUDGET` and the 90,000 ms whole-job ceiling. The slowest frozen normal
row is controlled-indicator seed 2 at 4,307 ms, below its unchanged 5,000 ms
ceiling; its warm receipt performs zero solver solves. The slowest D01-job row
is RB15 seed 0 at 13,994 ms, inside its stated 90,000 ms budget. The independent
reader passes on the captured report. Earlier failed timing probes are retained in
...
The rebuilt compiled Quick Play matrix passed all 30 menu cases (three
...
The matrix discovers nine EASY pairs (including SENSOR_CONTROL) and the composed
MEDIUM pair from the current menu; it does not rewrite the historical
nine-family/180-board qualification as if Sensor Control existed then.
