...
try:
 with sync_playwright() as p:
  ...
  try:
   ...
   flags={'p06':'tsjVerifyP06=true','negative':'tsjVerifyP06=true&tsjP06Fail=true','bench':'tsjVerifyP06=true&tsjP06Bench=true','layout':'tsjVerifyLayout=true&tsjVerifyGeometry=true','q15':'tsjVerifyQ15=true','q15negative':'tsjVerifyQ15=true&tsjQ15Fail=true'}
   ...
   report=page.locator('html').get_attribute('data-tsj-q15-report' if mode.startswith('q15') else 'data-tsj-p06-report')
   result['report']=json.loads(report) if report else None
   cleanup=page.locator('html').get_attribute('data-tsj-p06-cleanup')
   ...
   if mode=='bench':
    page.screenshot(path=str(evidence/'00-raised-link.png'),full_page=True)
    page.get_by_role('button',name='View bottom copper',exact=True).click()
    page.screenshot(path=str(evidence/'01-bottom-face.png'),full_page=True)
    page.get_by_role('button',name='View top copper',exact=True).click()
    coordinates=json.loads(page.locator('html').get_attribute('data-tsj-p06-body'))
    canvas=max(page.locator('canvas:visible').all(),key=lambda item:item.bounding_box()['width']*item.bounding_box()['height'])
    box=canvas.bounding_box(); width=int(canvas.get_attribute('width')); height=int(canvas.get_attribute('height'))
    x=box['x']+coordinates['x']*box['width']/width
    y=box['y']+coordinates['y']*box['height']/height
    page.mouse.click(x,y,button='right')
    menu=page.get_by_role('dialog',name='Component actions',exact=True)
    menu.wait_for(state='visible',timeout=10000)
    save('component-menu.json',{'text':menu.inner_text()})
    page.screenshot(path=str(evidence/'02-component-actions.png'),full_page=True)
    menu.get_by_role('button',name='Remove component',exact=True).click()
    page.wait_for_timeout(250)
    save('removed-dom.json',{'text':page.locator('body').inner_text()})
    page.screenshot(path=str(evidence/'03-removed-link.png'),full_page=True)
    page.get_by_role('button',name='Raised insulated factory link (50 mOhm)',exact=True).first.click()
    page.get_by_role('button',name='Install as FL1',exact=True).click()
    page.wait_for_timeout(250)
    page.get_by_role('button',name='OHM',exact=True).click()
    page.get_by_label('Probe terminal',exact=True).select_option('FL1.1')
    page.get_by_role('button',name='Place red probe',exact=True).click()
    page.get_by_label('Probe terminal',exact=True).select_option('FL1.2')
    page.get_by_role('button',name='Place black probe',exact=True).click()
    page.wait_for_timeout(500)
    meter_text=page.locator('body').inner_text()
    assert '50 mOhm' in meter_text.splitlines(),meter_text
    result['measuredLinkOhms']=0.05
    save('meter-dom.json',{'text':meter_text})
    page.screenshot(path=str(evidence/'04-restored-link-ohms.png'),full_page=True)
    result['inputs']=['view bottom copper','view top copper','right-click actual link body','remove component',
        'select loose physical link','install as FL1','OHM','place red on FL1.1','place black on FL1.2']
   page.screenshot(path=str(evidence/(mode+'.png')),full_page=True)
   expected = 'FAIL:q15:q15-explicit-failure-canary' if mode=='q15negative' else 'FAIL:P06 forced-negative' if mode=='negative' else 'PASS:'+('p06' if mode=='bench' else mode)
   ...
   if mode=='p06': assert result['report']['ownerRestored'] and result['report']['normalAdoption'] is False
