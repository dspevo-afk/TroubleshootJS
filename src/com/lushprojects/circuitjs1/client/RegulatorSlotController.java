...
/** Runtime graph and identity mutation controller for the replaceable U1 rail. */
final class RegulatorSlotController implements PhysicalSlotMutationProvider,
        PhysicalSlotMutationProvider.Scoped, CatalogAcquisitionProvider {
    private static final int START_X = 2400;
    private static final int START_Y = 3200;
    private static final int STEP = 64;

    private final CirSim sim;
    private final GeneratedBoardInstance instance;
    private final BoardModificationController modifications;
    private final ReplaceableRegulatorBoardCapability capability;

    RegulatorSlotController(CirSim sim, GeneratedBoardInstance instance,
        ...
        if (sim == null || instance == null || modifications == null || capability == null)
            throw new IllegalArgumentException("Missing regulator slot controller context");
        this.sim = sim;
    ...
    public WorkbenchCapabilityMetadata getMetadata() {
        return new WorkbenchCapabilityMetadata("REPLACEABLE_REGULATOR_" + getComponentId(),
            "Regulator workbench", "SLOT_OPERATIONS");
    }

    public String getOperationLabel(WorkbenchOperation operation) {
        if (operation == null) return "Modify regulator";
    ...
    public boolean isAvailable(WorkbenchOperation operation, WorkbenchCapabilityContext context) {
        ...
        RegulatorComponentSlot slot = capability.getSlot();
        String id = operation.getId();
        ...
        if (WorkbenchOperation.INSTALL.equals(id))
            return operation.getPart() instanceof PhysicalRegulatorPart &&
    ...
    public boolean invoke(WorkbenchOperation operation, WorkbenchCapabilityContext context) {
        ...
        if (WorkbenchOperation.REMOVE.equals(operation.getId()))
    ...
    public boolean install(String partId) {
        ...
        if (!(candidate instanceof PhysicalRegulatorPart) ||
                !instance.getPhysicalBoardRuntime().isPartInstallableAt(candidate,
                    getComponentId()))
            return false;
        PhysicalRegulatorPart part = (PhysicalRegulatorPart) candidate;
    ...
    private PhysicalRegulatorPart catalogPart(String catalogEntryId, boolean install) {
        requireSafeMutation();
        final RegulatorComponentSlot slot = capability.getSlot();
        if (install && !slot.isEmpty()) return null;
        final RegulatorCatalogEntry entry = capability.getCatalog().get(catalogEntryId);
        final RailRegulationContract contract = entry.getContract();
        if (contract != slot.getIntendedContract())
            throw new IllegalArgumentException("Regulator catalog changed the selected contract");
        final AbstractRailRegulatorElm element = createBacking(contract,
            instance.getSimulationElements());
        final String componentId = slot.getComponentId();
        PhysicalMutationScope scope = newScope(install ? "catalog" : "acquire", null,
            null, catalogEntryId);
        PhysicalRegulatorPart part;
        try {
            part = scope.acquire(capability.getInventory(), componentId + "_CATALOG_PART",
                new PhysicalPartIdentityFactory<PhysicalRegulatorPart>() {
                    public PhysicalRegulatorPart create(String partId) {
                        PhysicalRegulatorPart created = new PhysicalRegulatorPart(partId,
                            entry.getSpecification(),
                            entry.getPlayerVisibleNameplate().forPhysicalPartId(partId),
                            element, contract, new PhysicalPartProvenance(
    ...
    private void requireSafeMutation() {
        if (!isSafeMutationAvailable())
            throw new BoardModificationRejectedException(
                "Regulator replacement requires electrically unpowered generated board");
    }
    ...
    private void retargetComponentLeadBindings(PhysicalRegulatorPart part,
    ...
    private boolean hasCatalogEntry(String catalogEntryId) {
        ...
        for (RegulatorCatalogEntry entry : capability.getCatalog().getEntries())
            if (catalogEntryId.equals(entry.getId())) return true;
        return false;
    }

    /** Allocates a detached real rail element; all electrical behavior stays in CircuitJS. */
    private static AbstractRailRegulatorElm createBacking(RailRegulationContract contract,
            Vector<CircuitElm> occupiedElements) {
        if (contract == null || occupiedElements == null)
            throw new IllegalArgumentException("Missing regulator backing allocation context");
        for (int index = 0; ; index++) {
            int x = START_X + (index % 24) * STEP;
            int y = START_Y + (index / 24) * STEP;
            if (!occupied(occupiedElements, x, y, x + STEP, y) &&
                    !occupied(occupiedElements, x + STEP / 2, y - 32,
                        x + STEP / 2, y + 32)) {
                AbstractRailRegulatorElm element = contract.isAveragedSwitching() ?
                    new AveragedSwitchingRegulatorElm(x, y, contract) :
                    new LinearRegulatorElm(x, y, contract);
                element.x2 = x + STEP;
                element.y2 = y;
                element.setPoints();
                return element;
    ...
    private static boolean occupied(Vector<CircuitElm> elements, int firstX, int firstY,
        ...
        for (CircuitElm element : elements) {
            for (int post = 0; post < element.getPostCount(); post++) {
                ...
                if ((point.x == firstX && point.y == firstY) ||
                        (point.x == secondX && point.y == secondY)) return true;
            }
        }
        return false;
    }
