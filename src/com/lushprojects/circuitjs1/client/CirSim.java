...
//	useFrame = false;
	...
	boolean troubleshootTask48Verification;
...
//    String baseURL = "http://www.falstad.com/circuit/";
	...
	try {
	    ...
	    troubleshootTask48Verification = troubleshootDebug &&
		qp.getBooleanValue("tsjVerifyTask48", false);
	    troubleshootTask48ForcedFailure = troubleshootTask48Verification &&
	    ...
	    troubleshootA03Verification = troubleshootDebug &&
	...
	controlledIndicatorChallengeButton.addClickHandler(new ClickHandler() {
	    public void onClick(ClickEvent event) {
		if (!canOpenControlledIndicatorChallenge()) return;
		try {
		    openControlledIndicatorChallenge(System.currentTimeMillis());
		} catch (Throwable failure) {
		    Window.alert("The challenge could not be opened. Please try again.");
    ...
    private GeneratedBoardInstance generateNpnBoard(long seed) {
	try {
	    NpnLowSideSwitchGenerator generator = new NpnLowSideSwitchGenerator();
	    GeneratedFaultType forced = parseNpnFaultForVerification();
	    return forced == null ? generator.generate(seed) :
	        generator.generateForFaultVerification(seed, forced);
	} catch (RuntimeException failure) {
	    console("npn_generator_failure: " + failure.getMessage());
	    throw failure;
	}
    }
	...
	if (!troubleshootNpnVerification || troubleshootNpnFault == null)
	    return null;
	if ("TRANSISTOR_CE_OPEN".equals(troubleshootNpnFault))
	    return GeneratedFaultType.TRANSISTOR_CE_OPEN;
	if ("TRANSISTOR_CE_SHORT".equals(troubleshootNpnFault))
	    return GeneratedFaultType.TRANSISTOR_CE_SHORT;
	if ("BASE_RESISTOR_OPEN".equals(troubleshootNpnFault))
	    return GeneratedFaultType.BASE_RESISTOR_OPEN;
	if ("LOAD_PATH_OPEN".equals(troubleshootNpnFault))
...
//                     UPDATE CIRCUIT
    ...
    if (failedGeneratedRuntimeOwner != null &&
        ...
        return;
    GeneratedBoardInstance updateOwner = generatedBoardInstance;
    boolean wasGeneratedSettled = updateOwner != null && isGeneratedRuntimeSettled();
    long mystarttime;
    long myrunstarttime;
...
//	    mouseElm = (CircuitElm) (editDialog.elm);
	...
	if (isPcbWorkbenchVisible()) {
	    ...
	    frames++;
	    refreshSettledGeneratedControls(updateOwner, wasGeneratedSettled);
	    return;
	}
...
//	g.drawString("ms per frame (draw): "+ CircuitElm.showFormat.format((mydrawtime)/myframes),10,150);
    ...
    private void refreshSettledGeneratedControls(GeneratedBoardInstance updateOwner,
	    boolean wasGeneratedSettled) {
...
//    }
    ...
    private long parseControlledIndicatorSeed(String seedText) {
	if (seedText == null) return 0;
	long value = Long.parseLong(seedText);
	if (!Long.toString(value).equals(seedText))
	    throw new IllegalArgumentException("Controlled indicator requires a canonical signed seed");
	return value;
    }

    private boolean canOpenControlledIndicatorChallenge() {
	return !FreshGeneratedRuntimeInstallation.isInProgress(this) &&
	    !GeneratedDiagnosticSolvabilityAdmission.isInternalProofRunning() &&
	    isGeneratedRuntimeSettled() && !activeMeasurementOverlay &&
	    (generatedBoardInstance == null ||
    ...
    private void runP06FactoryLinkVerificationIfReady() {
        if (!developerVerifierRunning && troubleshootP06Verification && !troubleshootP06Complete &&
                !GeneratedDiagnosticSolvabilityAdmission.isInternalProofRunning() &&
                generatedChallengeController != null && generatedChallengeController.isReady() &&
                isGeneratedRuntimeSettled()) {
            developerVerifierRunning = true; troubleshootP06Complete = true;
            publishBrowserVerificationResult("RUNNING:p06");
            try {
                P06FactoryLinkDeveloperVerifier.verify(this, troubleshootP06Forced, troubleshootP06Bench);
                publishBrowserVerificationResult("PASS:p06");
            } catch (Throwable failure) {
                publishBrowserVerificationResult("FAIL:" + failure.getMessage());
                PhysicalMutationScope.rethrow(failure);
            } finally { developerVerifierRunning = false; }
    ...
    private void runDeveloperVerificationIfReady() {
		...
		try {
		    ...
		    if (troubleshootNpnVerification && !troubleshootNpnVerificationComplete) {
			troubleshootNpnVerificationComplete = true;
	    ...
	    if (!developerVerifierRunning && troubleshootTask48Verification &&
		!troubleshootTask48VerificationComplete &&
		!GeneratedDiagnosticSolvabilityAdmission.isInternalProofRunning() &&
		generatedChallengeController != null && generatedChallengeController.isReady() &&
		isGeneratedRuntimeSettled()) {
		developerVerifierRunning = true;
		troubleshootTask48VerificationComplete = true;
		publishBrowserVerificationResult("RUNNING:task48");
		try {
		    publishTask48Evidence(Task48DeveloperVerifier.verify(this,
			troubleshootTask48Seed, troubleshootTask48ForcedFailure));
		    publishBrowserVerificationResult("PASS:task48");
		} catch (Throwable failure) {
		    publishBrowserVerificationResult("FAIL:task48:" + failure.getMessage());
		    if (failure instanceof Error) throw (Error) failure;
		    if (failure instanceof RuntimeException) throw (RuntimeException) failure;
		    throw new IllegalStateException("Task 48 verification failed", failure);
		} finally {
		    developerVerifierRunning = false;
		}
	    }
	    ...
	    if (!developerVerifierRunning && troubleshootA03Verification &&
		!troubleshootA03VerificationComplete &&
		!GeneratedDiagnosticSolvabilityAdmission.isInternalProofRunning() &&
		generatedChallengeController != null && generatedChallengeController.isReady() &&
		isGeneratedRuntimeSettled()) {
		developerVerifierRunning = true;
		troubleshootA03VerificationComplete = true;
		publishBrowserVerificationResult("RUNNING:a03");
		try {
		    publishA03Evidence(A03IdentityDeveloperVerifier.verify(this,
			troubleshootA03ForcedFailure));
		    publishBrowserVerificationResult("PASS:a03");
		} catch (Throwable failure) {
		    publishBrowserVerificationResult("FAIL:a03:" + failure.getMessage());
		    if (failure instanceof Error) throw (Error) failure;
		    if (failure instanceof RuntimeException) throw (RuntimeException) failure;
            ...
            if (!developerVerifierRunning && troubleshootP01Verification && !troubleshootP01VerificationComplete &&
                    !GeneratedDiagnosticSolvabilityAdmission.isInternalProofRunning() &&
                    generatedChallengeController != null && generatedChallengeController.isReady() &&
                    isGeneratedRuntimeSettled()) {
                developerVerifierRunning = true; troubleshootP01VerificationComplete = true;
                publishBrowserVerificationResult("RUNNING:p01");
                try {
                    P01PhysicalPoseDeveloperVerifier.verify(this, troubleshootP01ForcedFailure);
                    publishBrowserVerificationResult("PASS:p01");
                } catch (Throwable failure) {
                    publishBrowserVerificationResult("FAIL:p01:" + failure.getMessage());
                    if (failure instanceof Error) throw (Error)failure;
                    if (failure instanceof RuntimeException) throw (RuntimeException)failure;
                    throw new IllegalStateException("P01 verification failed", failure);
                } finally { developerVerifierRunning = false; }
            }
	...
	ResistorSlotController getResistorSlotController(String componentId) {
	    ReplaceableResistorBoardCapability capability =
		ReplaceableResistorBoardCapability.find(generatedBoardInstance == null ? null :
		    generatedBoardInstance.getPhysicalBoardRuntime(), componentId);
	    return capability == null ? null : capability.getController();
	}
